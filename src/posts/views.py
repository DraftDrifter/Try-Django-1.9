...
def post_detail(request, slug=None):
    ...
    if instance.published > timezone.now().date() and request.user != instance.user and not request.user.is_superuser:
        raise Http404
...
#        queryset_list = user_drafts | active_list | user_past_posts
    if request.user.is_staff and not request.user.is_superuser:
        #queryset_list = Post.get_current_users_futureposts_drafts(request)
        queryset_list = Post.current_user.all(request.user)
